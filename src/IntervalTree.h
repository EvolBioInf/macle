...
template <class T, typename K = std::size_t> class Interval {
public:
  K start;
  K stop;
  T value;
...
template <class T, typename K> K intervalStart(const Interval<T, K> &i) {
  return i.start;
}

template <class T, typename K> K intervalStop(const Interval<T, K> &i) { return i.stop; }
...
template <class T, typename K = std::size_t> class IntervalStartSorter {
public:
...
template <class T, typename K = std::size_t> class IntervalTree {
...
public:
  IntervalTree<T, K>(const intervalTree &other)
      : intervals(other.intervals), left(other.left ? copyTree(*other.left) : nullptr),
        right(other.right ? copyTree(*other.right) : nullptr), center(other.center) {}
...
public:
  ...
  IntervalTree<T, K>(intervalVector &ivals, std::size_t depth = 16,
                     std::size_t minbucket = 64, K leftextent = 0, K rightextent = 0,
    ...
    if (depth == 0 || (ivals.size() < minbucket && ivals.size() < maxbucket)) {
      ...
      for (typename intervalVector::const_iterator i = ivals.begin(); i != ivals.end();
           ++i) {
      ...
      if (!lefts.empty()) {
        left = std::unique_ptr<intervalTree>(
            new intervalTree(lefts, depth, minbucket, leftp, centerp));
      }
      if (!rights.empty()) {
        right = std::unique_ptr<intervalTree>(
            new intervalTree(rights, depth, minbucket, centerp, rightp));
      }
    }
  }
