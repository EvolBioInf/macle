...
Fact *computeMLFact(Esa *esa) {
  ...
  mlf->fact = (size_t*)malloc(esa->n * sizeof(size_t));
  mlf->n = 0;
  size_t i=0;
  while(i < esa->n){
    mlf->fact[mlf->n++] = i;
...
uint64_t* computeFactPrefixSum(Fact *f) {
  ...
  uint64_t *ft = (uint64_t*)calloc(n,sizeof(uint64_t));
...
/* mlComplexity: match length complexity, with given window and interval */
Fact *mlComplexity(Sequence *seq, Esa *esa){
  size_t n = esa->n;
