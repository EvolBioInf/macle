#include "prelude.h"

#include "eprintf.h"

#include "lempelziv.h"
#include "periodicity.h"
#include "esa.h"
#include "list.h"

Periodicity *newPeriodicity(size_t b, size_t e, size_t l) {
  Periodicity *p = emalloc(sizeof(Periodicity));
  p->b = b;
  p->e = e;
  p->l = l;
  return p;
}

void printPeriodicity(Periodicity *p) {
  printf("(%zu,%zu,%zu)\n",p->b, p->e, p->l);
}

//TODO: efficient internal lcp, and lcs (longest common suffix) (Ohlebusch!)
//TODO: efficient O(n) algorithm for periodicities / runs

//naive: length of lcp of suffixes i and j of given seq (1-indexed)
size_t lcp(char *str, size_t n, size_t i, size_t j) {
  i--; j--;
  uint64_t k=0;
  while (MAX(i,j)+k < n && str[k+i]==str[k+j])
    k++;
  return k;
}

//naive: length of lcs of prefixes 1..i and 1..j of given seq (1-indexed)
size_t lcs(char *str, size_t i, size_t j) {
  i--; j--;
  int64_t k=0;
  while (((int64_t)MIN(i,j))-k>=0 && str[i-k]==str[j-k])
    k++;
  return k;
}

//max. periodicities in O(nlog(n)) (Algorithm 5.16), mainly for reference / comparison
Periodicity *getPeriodicities2(char *str, size_t n, size_t *plen) {
  //Theorem 5.3.43 (upper bound for num. max. periodicities = c2 / 2 * n, c2=4.1)
  Periodicity *ps = emalloc(((size_t)2.05*n)*sizeof(Periodicity));
  *plen = 0;

  for (size_t l=1; l<n/2; l++) {
    size_t i = 2*l+1;
    while (i<=n+1) {
      size_t L = lcs(str, i-1-l, i-1);
      size_t R = lcp(str, n, i-l, i);
      if (L+R >= l && R<l) {
        ps[*plen].b = i-l-L;
        ps[*plen].e = i-1+R;
        ps[*plen].l = l;
        (*plen)++;
      }
      i+=l;
    }
  }
  return ps;
}

//length of factor
static inline size_t factLen(Fact *f, size_t i) {
  if (i==0)
    return f->fact[1];
  if (i==f->n-1)
    return f->strLen-f->fact[f->n-1];
  return f->fact[i+1]-f->fact[i];
}

//start position (1-indexed)
static inline size_t factStart(Fact *f, size_t i) {
  return f->fact[i]+1;
}

//end position (1-indexed)
static inline size_t factEnd(Fact *f, size_t i) {
  return (i < f->n-1 ? f->fact[i+1] : f->strLen);
}

//max. periodicities in O(n) using LZ-Factors
//TODO: adapt to filter only runs
Periodicity *getPeriodicities(Fact *lzf, size_t *plen) {
  //as required, array of lists of max. per. of type 1
  //indexed by start position, each sorted by end position
  List **Lt1 = ecalloc(lzf->strLen, sizeof(List*));
  //Algorithm 5.17
  for (size_t j=1; j<lzf->n; j++) {
    size_t sjLen = factLen(lzf,j);     // |s_j|
    size_t sjm1Len = factLen(lzf,j-1); // |s_(j-1)|
    size_t bj = factStart(lzf, j);     // b_j
    size_t bjm1 = factStart(lzf, j-1); // b_(j-1)
    size_t ej = factEnd(lzf, j);       // e_j
    size_t ejm1 = factEnd(lzf, j-1);   // e_(j-1)

    size_t max = MIN(sjLen+sjm1Len-1, ejm1);
    for (size_t l=1; l<=max; l++) {
      size_t L = lcs(lzf->str, ejm1-l, ejm1);
      size_t R = lcp(lzf->str, lzf->strLen, bj-l, bj);
      if (L+R >= l && (R>=1 || bj-l-L>bjm1)) {
        Periodicity *p = newPeriodicity(bj-l-L, ejm1+R, l);
        listAppend(&Lt1[p->b], p);
      }
    }

    for (size_t l=1; l<=sjLen; l++) {
      size_t L = lcs(lzf->str, ejm1, ejm1+l);
      size_t R = lcp(lzf->str, lzf->strLen, bj, bj+l);
      if (L+R >= l && bj+l-1+R<=ej && L < l) {
        Periodicity *p = newPeriodicity(bj-L, ejm1+l+R, l);
        listAppend(&Lt1[p->b], p);
      }
    }
  }

  //Algorithm 5.18
  for (size_t j=1; j<lzf->n; j++) {
    size_t bj = factStart(lzf, j);     // b_j
    size_t ej = factEnd(lzf, j);       // e_j
    if (factLen(lzf, j) >= 4) {
      printf("TODO type2 j=%zu\n",j);
      /* size_t dj = bj - prevOcc(j); //TODO */
      /* for (size_t i=bj+1; i<=ej-1; i++) { */
      /*   for each () { */
      /*     if */
      /*   } */
      /* } */
    }
  }

  //collect results from lists
  //Theorem 5.3.43 (upper bound for num. max. periodicities = c2 / 2 * n, c2=4.1)
  Periodicity *ps = emalloc(((size_t)(2.05*lzf->strLen))*sizeof(Periodicity));
  *plen = 0;
  for (size_t i=0; i<lzf->strLen; i++) {
    List *curr = Lt1[i];
    if (curr) {
      do {
        ps[(*plen)++] = *(Periodicity*)(curr->value);
        curr = curr->next;
      } while(curr);
    }

  }
  return ps;
}
